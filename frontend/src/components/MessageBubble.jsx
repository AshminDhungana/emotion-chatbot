...
export default function MessageBubble({ text, sender, timestamp }) {
  ...
  return (
    <div
      className={`flex ${isUser ? "justify-end" : "justify-start"} my-3 px-2 animate-fadeIn`}
    >
      <div className={`flex items-end gap-2 max-w-[75%] ${isUser ? "flex-row-reverse" : "flex-row"}`}>
        ...
        <div
          className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
            isUser ? "bg-indigo-500 text-white" : "bg-gray-300 text-gray-700"
          }`}
        >
        ...
        <div className="flex flex-col gap-1">
          <div
            className={`px-4 py-2 rounded-2xl text-sm break-words break-all shadow-sm transition-colors duration-200 ${
              isUser
